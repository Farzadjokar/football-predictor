...
OUT = "out_csv"        # output folder for the CSV tables
...
def discover_comp_season_pairs(root: Path) -> list[tuple[int, int]]:
    ...
    return pairs
# 2. Matches (one file per competition_id/season_id)
...
def load_lineups(root: Path, match_ids: list[int]) -> pd.DataFrame:
    ...
    return lineups
# 4. Events (one file per match_id) -- the big table
_EVENT_COLS = [
    ...
    "pass", "shot", "carry", "duel", "pressure", "dribble",
    "foul_committed", "bad_behaviour",
]
...
def load_events(root: Path, match_ids: list[int]) -> pd.DataFrame:
    ...
    # STRICT ordering, required for the time-t snapshot cut later
    # timestamp is 'HH:MM:SS.mmm' within each period; sort by
    # (match_id, period, timestamp, index) so event order within a match
    # is unambiguous and monotonic -- this is the ordering Stage 3
    # (Preprocessing) and Stage 5B (in-play snapshotting) both depend on.
    events = events.sort_values(["match_id", "period", "timestamp", "index"]).reset_index(drop=True)

    log.info("events -> %d rows from %d matches (%.0f events/match on average)",
...
def build_relational_store(root: Path, out_dir: Path,
    ...
    # sanity: every event/lineup match_id must exist in matches (referential integrity)
    ...
    for name, df in tables.items():
        ...
        log.info("wrote %s (%d rows, %d cols) -> %s", name, len(df), df.shape[1],
                  out_dir / f"{name}.csv")
    return tables
# Run
root_path = Path(ROOT);
